...
function paginate_type(page_number) {
...
async function display_by_type() {
    var selected_type = $("#poke_type option:selected").val()
    await $.ajax(
        {
            "url": `https://pacific-refuge-45875.herokuapp.com/type/${selected_type}`,
            "type": 'GET',
...
function insertSearchEventToTheTimeLine(poke_type) {
    $.ajax({
        ...
        type: "put",
        data: {
            text: ` Client has searched for Type ID:${poke_type}`,
            time: `${time}`,
            hits: 1
        },
        success: function (r) {
...
$(document).ready(function () {
    ...
    $("#search").click(() => {
        poke_name = $("#poke_name").val() - 1
        insertSearchEventToTheTimeLine($("#poke_name").val())
        $.ajax(
            {
                "url": `https://pacific-refuge-45875.herokuapp.com/${poke_name}`,
                "type": 'GET',
                "success": function process(data) {
                    poke_name = data.name
                    poke_id = data.id
